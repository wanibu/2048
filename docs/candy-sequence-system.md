...
下面这部分不是当前代码现状，而是已经确认的下一步协议方向。

### 1. `start-game` 不再一次性返回完整 sequence

后续准备改成：

- 开局时接口不再返回整条完整 `sequence`
- 而是只返回本局初始化所需的一小段 token
- 前端界面最终只需要维护两个可发射糖果位：
  - 当前糖果
  - 下一个糖果预览
...
- 每次请求最多先返回 `2` 个 token
- 正常情况下，这 `2` 个 token 会用于补足：
  - 当前糖果
  - 下一个糖果
- 玩家每打出去一个糖果，就再请求下一组 token

也就是说，前端不再持有一整条完整序列，而是按操作节奏持续向后端取小批量 token。

### 3. `stone` 不是立即执行，而是按消费时机触发

这是已经确认的规则：

- 如果返回的 token 里出现 `"stone"`
- 前端不能在收到 API 响应时立刻把石头放到棋盘上
- 前端必须先把这个 `"stone"` 记入本地待消费队列
- 等序列消费推进到它的真实时机时，再触发石头生成
...
### 4. 若本次返回中包含 `stone`，继续请求直到补满两个可发射糖果位
...
而不是“API 一返回 stone 就立即落盘”。

### 5. 签名只在 `action` 推进，不在取 token 时推进
